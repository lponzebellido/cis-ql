...
public:
    ...
    BinaryConditionNode(std::string o, std::unique_ptr<ConditionNode> l, std::unique_ptr<ConditionNode> r)
...
public:
    ...
    NotConditionNode(std::unique_ptr<ConditionNode> cond) : condition(std::move(cond)) {}
    void print(int indent = 0) const override;
};
...
public:
    ...
    ExtractStmtNode(std::string e, std::unique_ptr<ConditionNode> w) : entity(e), whereClause(std::move(w)) {}
    void print(int indent = 0) const override;
};

class SetOpStmtNode : public StatementNode {
public:
    std::string op; // INTERSECT, UNION, EXCEPT
    std::string entity1;
    std::string entity2;
    std::unique_ptr<ConditionNode> whereClause;
    SetOpStmtNode(std::string o, std::string e1, std::string e2, std::unique_ptr<ConditionNode> w)
